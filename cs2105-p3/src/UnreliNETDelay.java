...
public class UnreliNETDelay {
			...
			try {
				...
				while (true) {
					...
					if (is_delaying) {
						if (new Date().getTime() - delay_start <= delay_value) {
							for (Integer dst_port : dst_port_list) {
								DatagramPacket out_pkt = new DatagramPacket(
										in_data, in_pkt.getLength(), dst_addr,
										dst_port);
							...
							while (delay_pkt_list.isEmpty() != true)
								sk_out.send(delay_pkt_list.poll());
						}
						...
						for (Integer dst_port : dst_port_list) {
							DatagramPacket out_pkt = new DatagramPacket(
									in_data, in_pkt.getLength(), dst_addr, dst_port);
							delay_pkt_list.addLast(out_pkt);
						}
						System.out.println("Packet Delayed");
	...
	public UnreliNETDelay(int sk1_dst_port, int sk2_dst_port, int sk3_dst_port) {
	...
	public static void main(String[] args) {
		...
		if (args.length != 3) {
			...
			new UnreliNETDelay(Integer.parseInt(args[0]),
